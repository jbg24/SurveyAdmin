...
parser.add_argument('--ignorePeriod', dest='ignorePeriod', action = 'store_true',
...
def checkMissing(roster_df,dropPeriod):
    ...
    :return: (string) Places where there are missing values
    '''
    ...
    row_list = []
    if len(roster_df[pd.isnull(roster_df).any(axis=1)]) > 0:
        print >> sys.stdout, "\n*****Missing Values*****\n"
        row_list = roster_df[pd.isnull(roster_df).any(axis=1)].index
        print >> sys.stdout, roster_df[roster_df.index.isin(row_list)]
        print >> sys.stdout, "\n"
        #for rownum in roster_df[pd.isnull(roster_df).any(axis=1)].index:
        #    row_list.append(roster_df[roster_df.index.isin(rownum)])

        #print >> sys.stdout, "*****Missing Values*****"
        #for row in row_list:
        #    print >> sys.stdout, row
    #if row_list:
    #    issue = "There are missing values in rows: {}".format(', '.join(row_list))
    #    print roster_df.index.isin(row_list)
    #return issue
...
def clean(roster_df,ignorePeriod,clean_name,summ_name):
    '''
    ...
    #has_missing = checkMissing(roster_df,ignorePeriod)
    #if has_missing:
    #    failIssues.append(has_missing)
    ...
    summarize(roster_df,summ_name)

    roster_df.to_csv(clean_name,index=False)

def summarize(roster_df,summ_name):
    '''
    ...
    r_grouped = r_grouped.sort('StudentID')
    if min(r_grouped['StudentID'].tolist()) < 5:
        print >> sys.stdout, "\nThe following courses have too few students:\n"
        print >> sys.stdout, r_grouped[r_grouped['StudentID']<5]
...
if __name__ == "__main__":
    ...
    for infile in args.inFile:
        ...
        clean(roster,args.ignorePeriod,clean_name,summ_name)
